...
* 도커는 가상머신과 유사하지만, 각각의 운영체제를 가지지는 않는다. 다만 가상 컨테이너를 통해 자원을 분리하고, 호스트의 운영 체제 커널을 공유한다.
