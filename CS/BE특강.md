...
* 처음부터 잘 만드는 것이 중요하다.
* Dual Write: 무중단 DB 마이그레이션 전략. 서비스를 운영하면서, 데이터 구조를 변경하거나 마이그레이션 하는 전략.
### TPS
...
### 시스템 구조 설계
* 컴포넌트 아키텍처: 시스템 구성요소를 배포할 수 있는 가장 작은 단위인 컴포넌트의 상호작용을 정의
* 데이터 아키텍처: 데이터의 수집, 변환, 분배, 소비등의 데이터 관리 방식을 정의
* 네트워크 아키텍처: 네트워크의 물리적인 요소들과 기능 조직, 구성, 사용되는 통신 프로토콜의 사양을 정의
...
* was는 복잡하고 에러가 날 확률이 높다.
* 그렇기에 정적 페이지를 반환해줄 때는 웹서버를 분리해서 쓴다.
* 이렇게 분리하는 경우, was가 작동하지 않아도, ws에서 일부 페이지를 보여 줄 수 있다.
### Apache vs Nginx
*  사용자는 서버의 80번포트에 연결을 요청한다.
* 많은 요청이 들어오면 어떻게 처리할까?
* Apache WS
  * 초기버전
    * 요청이 들어올 때 마다, fork()를 사용하여 자식 프로세스 생성
    * 근데 fork()는 좀 느리다.
    * 그럼 프로세스 말고 스레드 쓰면 되지 않나...?
  * 신형 버전
    * Hybrid model: 여러 chile Process를 미리 만들고, 각 Process들이 thread를 생성
* Nginx
  * 요청을 꼭 기다려야하나?
  * 논블로킹 I/O를 사용. 메인 프로세스는 요청이 들어오면 토스만 하고 다른 일을 한다.
  * 대신, 쓰레드풀을 만들어놓고, 수행햐아 하는 일이 생기면 여기서 처리한다.
...
* 한번에 10000개의 클라이언트를 동시에 처리할 수 있는 네트워크 I/O를 설계할수있나?
  * Apache: 요청이 들어올 때 마다 쓰레드 생성
  * Nginx: 요청이 들어왔다고 바로 처리하지 않고, 논블로킹으로 넘김
  * 10000개의 쓰레드를 실제로 만드는 것은 불가능하기에... 이런 관점에서 Nginx가 유리하다.
...
## TCP/UDP
### HTTP
* W3상에서 정보를 주고받을 수 있는 프로토콜. 주로 HTML문서를 주고받는 데에 쓰인다. 주로 TCP를 사용한다. HTTP/3부터는 UDP 사용
### TCP와 UDP 차이
* TCP : 신뢰성 있는 통신
* UDP : 신뢰성 없지만 빠르다...
이게 다일까?
...
* TCP통신은 1:1로 이루어진다! (유니캐스트이다.) 출발지 IP에 대한 정보도 없다..
...
* 브라우저가 TCP인지 UCP인지 어떻게 아나..? 같은 문제들이 있긴하다
  * 브라우저의 해결법 : TCP요청을 먼저 보내고, 'alt-svc' 헤더를 통해 확인, TCP UDP요청을 동시에 보내는 등
## 서버 늘리기
### Scale Out, Scale Up
* Scale Out : 서버 대수 늘리기
* Scale Up : 서버의 사양 늘리기
### DB 서버는 확장이 쉽지 않다.
* 요청에 의해 쓰여진 정보가 DB서버마다 달라지면? 이걸 해결해야한다.
...
### DNS
* 요청마다 다른 IP를 준다면? 
* 이를 통해 DNS Round Robin, DNS를 통해 로드밸런싱 할 수 있다.
...
* 그렇기에 캐시 메모리만으로 커다란 성능 향상을 노리기엔 쉽지 않다.
### 메모리를 캐시로 쓰자!
* 아무리 SSD 기술이 발전해도, 램의 I/O성능을 이기기는 힘들다.
* 그래서 나온게 캐시DB(레디스). 메모리 위에 존재하여, 성능이 좋다.
### Redis?
* in-memory data store. cache, message broker등으로 사용
* 그래서 접근 속도가 빠르다...
* 굳이 레디스 쓰지 않고 서버 내부 램 쓰는게 몇십배는 더 빠르지 않을까?
  * 특히 서버가 1대라면
### Local Cache, Shared Cache
* 불필요하게 여러 서버가 똑같은 캐시 데이터를 Local Cache에 저장 할 필요 없다.
* Redis가 Shared Cache역할을 할 수 있지 않을까?
* 하지만 Shared Cache와 Local Cache의 공존은 쉽지 않다.
  * 여러 LocalCache와 SharedCache의 정합성을 맞추기 쉽지 않다.
  * 정합성을 약간 희생해도 되거나, 정말 해당 LocalServer에서만 쓰인다면 LocalCache, 정합성이 중요하고 여러 서버에서 공유하는 데이터라면 SharedCache에 넣자
### Cache Hit, Cache Miss
* 캐시 미스가 너무 높으면, 오히려 안쓰느니만 못하다.
* 대책1: 캐시 메모리 늘리기(Redis 메모리 늘리기). 이는 너무 많은 비용 소모
* 대책2: Cache Hit율 높이기: 페이지 교체 알고리즘을 잘 써서 높여보자.
  * LRU알고리즘을 사용한다.
    * 사실 LRU가 아닌 Clock을 사용한다.
* 혹은 데이터의 손실이 발생해도 문제가 없고, 데이터가 크지 않다면 local cache를 사용하는것도 좋다.
* 혹은 캐시 친화적인 로직을 설계해보자.
### 메모리를 희생해서 성능을 끌어올릴 수 있다면?
* PVM, JVM: JIT 컴파일러(PVM은 Pypy), 자주 쓰이는 부분만 컴파일한다.
### case study: MySQL Buffer Pool
* SSD를 쓰면 DBMS가 빨라질까?
* InnoDB는 인메모리에 캐싱을 한다.
* 그래서 버퍼사이즈(메모리크기)가 커질수록 SSD와 HDD의 성능 차이가 적어진다.
